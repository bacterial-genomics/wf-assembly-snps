...
if [ "${reference_path}" == 'largest' ]; then
  largest_size=$(ls -SLr ./.tmp/* | tail -n 1 | xargs basename)
  reference_path=./.ref/"${largest_size}"
  mv -f ./.tmp/"${largest_size}" "${reference_path}"
