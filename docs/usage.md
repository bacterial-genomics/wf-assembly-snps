...
First, go to the [wf-assembly-snps releases page](https://github.com/bacterial-genomics/wf-assembly-snps/releases) and find the latest pipeline version - numeric only (eg. `1.0.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.0.1`. Of course, you can switch to another version by changing the number after the `-r` flag.
